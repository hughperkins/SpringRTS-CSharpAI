...
namespace CSharpAI
{
    ...
    {
        static BuildTree instance = new BuildTree();
        public static BuildTree GetInstance(){ return instance; }

        Dictionary<string, List<string>> buildablenamesbyname = new Dictionary<string, List<string>>();
        ...
        public bool CanBuild(string buildername, string targetname)
        {
            if (!buildablenamesbyname.ContainsKey(buildername))
            {
                ...
                {
                    thisbuilderoptions.Add( buildoption.UnitTypeName.ToLower() );
                }
            }
