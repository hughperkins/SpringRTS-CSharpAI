...
namespace CSharpAI
{
    ...
    class ConstructorController : IUnitRequester, IController, IFactoryController
    {
       ...
       // FactoryController factorycontroller = null;
        ...
        {
            if( UnitDefByUnitId.Count < optimalnumberconstructors && ( factory.Name == "armvp" || factory.Name == "armlab" ) )
            {
        ...
        public IUnitDef WhatUnitDoYouNeed( IFactory factory )
        {
            ...
            {
                unitdef = buildtable.UnitDefByName[ "armcv" ] as IUnitDef;
            }
        ...
        public void UnitFinished( int deployedunitid, IUnitDef unitdef )
        {
            //if( unitdef.name.ToLower() == "armcv" || unitdef.name.ToLower() == "armck" || unitdef.name.ToLower() == "armca" || 
            //    unitdef.isCommander )
            //{
              //  if( !UnitDefByUnitId.Contains( deployedunitid ) )
               // {
                ...
                //}
            //}
            if( unitdef.isCommander && unitdef.name.ToLower() != "armcom" )
            {
